...
// app.use(cors({
//   origin: ['https://www.jobneura.tech', 'https://jobneura.tech'], // frontend origin
//   credentials: true,
// }));
...
import session from 'express-session';
import cookieParser from 'cookie-parser';
import passport from './config/passport.js'; // your GitHub/Google strategies
import User from './models/User.js';
import { protect, requireRole } from './middlewares/authMiddleware.js';
...
// CORS config to permit your production front-end and handle preflight
const allowedOrigins = [
  'https://www.jobneura.tech',
  'https://jobneura.tech',
  'http://localhost:5173', // keep dev for local testing
];
const corsOptions = {
...
app.use(session({
  ...
  cookie: {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production', // serve only over HTTPS
    sameSite: process.env.NODE_ENV === 'production' ? 'None' : 'Lax',
    maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
  },
}));
